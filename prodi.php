...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $response = apiRequest(API_PRODI, 'POST', $data, $token);
        if ($response['success']) {
            $message = 'Prodi berhasil ditambahkan';
            $messageType = 'success';
        } else {
            $message = $response['message'] ?? 'Gagal menambahkan prodi';
            $messageType = 'danger';
        }
        ...
        $response = apiRequest(API_PRODI . '?id=' . $id, 'PUT', $data, $token);
        if ($response['success']) {
            $message = 'Prodi berhasil diupdate';
            $messageType = 'success';
        } else {
            $message = $response['message'] ?? 'Gagal mengupdate prodi';
            $messageType = 'danger';
        }
    } elseif ($action === 'delete') {
        $id = $_POST['id'] ?? '';
        $response = apiRequest(API_PRODI . '?id=' . $id, 'DELETE', null, $token);
        if ($response['success']) {
            $message = 'Prodi berhasil dihapus';
            $messageType = 'success';
        } else {
            $message = $response['message'] ?? 'Gagal menghapus prodi';
            $messageType = 'danger';
        }
    }
}
...
// Get all prodi
$response = apiRequest(API_PRODI, 'GET', null, $token);
$prodi = $response['data'] ?? [];
...
<head>
    ...
    <title>Manajemen Prodi - Admin</title>
...
<body>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><i class="bi bi-diagram-2"></i> Manajemen Prodi</h1>
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalProdi" onclick="resetForm()">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php if (empty($prodi)): ?>
                                        <tr>
                                            <td colspan="4" class="text-center text-muted">Tidak ada data prodi</td>
                                        </tr>
                                    <?php else: ?>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <button class="btn btn-sm btn-warning" onclick="editProdi(<?php echo htmlspecialchars(json_encode($p)); ?>)">
    ...
    <script>
        ...
        function editProdi(data) {
            ...
            const modal = new bootstrap.Modal(document.getElementById('modalProdi'));
            modal.show();
        }
    </script>
</body>
